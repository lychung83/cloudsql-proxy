...
// Package clientcategory provides facilities to get the category of running cloudsql-proxy.
...
// OnGKE checks whether the client is running on GKE or not.
func OnGKE() bool {
	onGKEOnce.Do(initOnGKE)
	return onGKE
}
...
// initOnGKE implements OnGKE. We use the presence of kubernetes specific env variable to tell
// whether we're in GKE or not. For simlar uses, see
// https://github.com/kubernetes/client-go/blob/ca74aedde1ee2ea0ae6a39c3fb58409c1f04840f/rest/config.go#L320-L323
func initOnGKE() {
	onGKE = os.Getenv("KUBERNETES_SERVICE_HOST") != "" && os.Getenv("KUBERNETES_SERVICE_PORT") != ""
}
