...
func myCopy(ctx context.Context, ipTypeTag octag.Tag, instance string, metric *metrics.Metric, dst io.Writer, src io.Reader) (readErr bool, err error) {
	buf := make([]byte, 4096)
	for {
		n, err := src.Read(buf)
		if n > 0 {
			wBytes, werr := dst.Write(buf[:n])
			if recErr := record.SetVal(ctx, metric, int64(wBytes), ipTypeTag); recErr != nil {
...
func copyThenClose(ctx context.Context, ipTypeTag octag.Tag, remote, local io.ReadWriteCloser, instance, localDesc string) (termCode string) {
	firstErr := make(chan error, 1)
	termCodeCh := make(chan string, 1)

	go func() {
		readErr, err := myCopy(ctx, ipTypeTag, instance, metrics.SentBytes, remote, local)
		select {
		case firstErr <- err:
			if readErr && err == io.EOF {
				termCodeCh <- termcode.OK
				logging.Verbosef("Client closed %v", localDesc)
			} else {
				if readErr {
					termCodeCh <- termcode.ClientReadErr
				} else {
					termCodeCh <- termcode.InstanceWriteErr
				}
	...
	readErr, err := myCopy(ctx, ipTypeTag, instance, metrics.RcvBytes, local, remote)
	select {
	case firstErr <- err:
		if readErr && err == io.EOF {
			termCodeCh <- termcode.InstanceClose
			logging.Verbosef("Instance %v closed connection", instance)
		} else {
			if readErr {
				termCodeCh <- termcode.InstanceReadErr
			} else {
				termCodeCh <- termcode.ClientWriteErr
			}
