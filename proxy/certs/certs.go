...
func NewCertSourceOpts(c *http.Client, opts RemoteOpts) *RemoteCertSource {
	...
	return &RemoteCertSource{pkey, serv, opts.IPAddrTypeOpts}
}
...
func (s *RemoteCertSource) Local(instance util.Instance) (ret tls.Certificate, err error) {
	pkix, err := x509.MarshalPKIXPublicKey(&s.key.PublicKey)
	if err != nil {
		return ret, err
	}

	req := s.serv.SslCerts.CreateEphemeral(instance.Project, instance.Database,
	...
	err = backoffAPIRetry("createEphemeral for", instance.String(), func() error {
		data, err = req.Do()
		return err
	})
	if err != nil {
...
func (s *RemoteCertSource) Remote(instance util.Instance) (cert *x509.Certificate, addr, name string, err error) {
	req := s.serv.Instances.Get(instance.Project, instance.Database)

	instName := instance.String()
	var data *sqladmin.DatabaseInstance
	err = backoffAPIRetry("get instance", instName, func() error {
		data, err = req.Do()
		return err
	})
	if err != nil {
	...
	if data.Region != instance.Region {
		return nil, "", "", fmt.Errorf(`for connection string "%v": got region %q, want %q`, instance, instance.Region, data.Region)
	}
	...
	// Find the first matching IP address by user input IP address types
	ipAddrInUse := ""
	ipAddrInUse, err = s.findIPAddr(data, instName)
	if err != nil {
		return nil, "", "", err
	}
	...
	return c, ipAddrInUse, instance.Project + ":" + instance.Database, err
}
