...
var mandatoryKeys = []octag.Key{tag.ProjectKey, tag.RegionKey, tag.DatabaseKey}
...
func Initialize() error {
	for _, m := range metricList {
		v := &view.View{
			...
			// TagKeys should contain all tag keys of the metric, so we add mandatory keys.
			TagKeys:     append(mandatoryKeys, m.Keys...),
			Measure:     m.Measure,
			Aggregation: m.Agg,
		}
